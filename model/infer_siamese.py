...
class SiamesePredictor:
    ...
    def __init__(self, model: torch.nn.Module, device: torch.device, 
                 image_size: int = 64, similarity_threshold: float = 0.4488):
        """
    ...
    def preprocess_image(self, image: Image.Image) -> torch.Tensor:
        ...
        try:
            ...
            threshold = np.mean(img_array)  # Utilise la moyenne comme dans create_templates.py
    ...
    def compare_images(self, img1_tensor: torch.Tensor, img2_tensor: torch.Tensor) -> float:
    ...
    def find_closest_symbol(self, image_path: Path) -> Tuple[str, float]:
        ...
        for symbol, similarity in sorted(similarities.items(), key=lambda x: x[1], reverse=True)[:5]:  # Top 5 seulement
