...
from model.siamese_model import ContrastiveLoss, SiameseNetwork
...
class TestContrastiveLoss:
    ...
    def test_loss_similar_pairs(self, device):
        ...
        assert (
            loss.item() < 1e-6
        )  # La perte doit être proche de 0 pour des embeddings identiques
    ...
    def test_loss_dissimilar_pairs(self, device):
        ...
        assert loss.item() >= 0  # La perte doit être positive
        assert (
            loss.item() < criterion.margin
        )  # La perte doit être inférieure à la marge
