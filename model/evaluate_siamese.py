...
class SiameseEvaluator:
    ...
    def compute_distances_and_labels(self, test_loader: DataLoader) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def find_optimal_threshold(self, distances: np.ndarray, labels: np.ndarray) -> Tuple[float, float]:
        """
    ...
    def plot_precision_recall_curve(self, distances: np.ndarray, labels: np.ndarray,
    ...
    def evaluate_with_threshold(self, distances: np.ndarray, labels: np.ndarray,
...
def main():
    ...
    logging.info(f"Seuil optimal trouvé : {best_threshold:.4f} (F1-score : {best_f1:.4f})")
