...
def check_duplicate_verre(db, verre_data, fournisseur, materiau, gamme, serie):
    ...
    query = db.query(Verre).filter(
        and_(
            ...
            Verre.variante
            == (verre_data["variante"] if pd.notna(verre_data["variante"]) else None),
            Verre.hauteur_min
            == (
                verre_data["hauteur_min"]
                if pd.notna(verre_data["hauteur_min"])
                else None
            ),
            Verre.hauteur_max
            == (
                verre_data["hauteur_max"]
                if pd.notna(verre_data["hauteur_max"])
                else None
            ),
            Verre.indice
            == (verre_data["indice"] if pd.notna(verre_data["indice"]) else None),
            Verre.url_gravure
            == (
                verre_data["url_gravure"]
                if pd.notna(verre_data["url_gravure"])
                else None
            ),
            Verre.url_source
            == (
                verre_data["url_source"] if pd.notna(verre_data["url_source"]) else None
            ),
...
def process_csv_file(db, csv_file: Path, global_stats: dict) -> None:
    ...
    try:
        ...
        file_stats = {"total": len(df), "importes": 0, "doublons": 0}

        # Création des traitements standards
        traitement_protection = get_or_create(
            db, Traitement, nom="Protection UV", type="protection"
        )
        traitement_photochromique = get_or_create(
            db, Traitement, nom="Photochromique", type="photochromique"
        )
        ...
        for _, row in df.iterrows():
            ...
            materiau = (
                get_or_create(db, Materiau, nom=row["materiau"])
                if pd.notna(row["materiau"])
                else None
            )
            gamme = get_or_create(db, Gamme, nom=row["gamme"])
            serie = (
                get_or_create(db, Serie, nom=row["serie"])
                if pd.notna(row["serie"])
                else None
            )

            # Vérification des doublons
            existing_verre = check_duplicate_verre(
                db, row, fournisseur, materiau, gamme, serie
            )
            ...
            verre = Verre(
                ...
                hauteur_min=(
                    row["hauteur_min"] if pd.notna(row["hauteur_min"]) else None
                ),
                hauteur_max=(
                    row["hauteur_max"] if pd.notna(row["hauteur_max"]) else None
                ),
                indice=row["indice"] if pd.notna(row["indice"]) else None,
                url_gravure=(
                    row["url_gravure"] if pd.notna(row["url_gravure"]) else None
                ),
...
def import_data() -> None:
    ...
    try:
        ...
        global_stats = {"total": 0, "importes": 0, "doublons": 0}
        ...
        with get_db() as db:
            ...
            for csv_file in enhanced_data_path.glob("*.csv"):
                ...
                try:
                    ...
                    for _, row in df.iterrows():
                        try:
                            ...
                            if fournisseur_nom not in fournisseurs_cache:
                                fournisseurs_cache[fournisseur_nom] = get_or_create(
                                    db, Fournisseur, nom=fournisseur_nom
                                )
                            ...
                            if pd.notna(row["materiau"]):
                                if row["materiau"] not in materiaux_cache:
                                    materiaux_cache[row["materiau"]] = get_or_create(
                                        db, Materiau, nom=row["materiau"]
                                    )
                                materiau = materiaux_cache[row["materiau"]]

                            if row["gamme"] not in gammes_cache:
                                gammes_cache[row["gamme"]] = get_or_create(
                                    db, Gamme, nom=row["gamme"]
                                )
                            ...
                            if pd.notna(row["serie"]):
                                if row["serie"] not in series_cache:
                                    series_cache[row["serie"]] = get_or_create(
                                        db, Serie, nom=row["serie"]
                                    )
                            ...
                            verre = Verre(
                                ...
                                variante=(
                                    row["variante"]
                                    if pd.notna(row["variante"])
                                    else None
                                ),
                                hauteur_min=(
                                    row["hauteur_min"]
                                    if pd.notna(row["hauteur_min"])
                                    else None
                                ),
                                hauteur_max=(
                                    row["hauteur_max"]
                                    if pd.notna(row["hauteur_max"])
                                    else None
                                ),
                                indice=(
                                    row["indice"] if pd.notna(row["indice"]) else None
                                ),
                                url_gravure=(
                                    row["url_gravure"]
                                    if pd.notna(row["url_gravure"])
                                    else None
                                ),
                                url_source=(
                                    row["url_source"]
                                    if pd.notna(row["url_source"])
                                    else None
                                ),
                                fournisseur=fournisseur,
                                materiau=materiau,
                            ...
                            db_logger.error(
                                f"Erreur lors du traitement d'une ligne : {str(e)}"
                            )
                            db.rollback()
                            continue
                    ...
                    db_logger.error(
                        f"Erreur lors du traitement du fichier {csv_file.name}: {str(e)}"
                    )
                    db.rollback()
                    continue
