...
def test_create_symbole(
    client: TestClient, db_session: Session, auth_headers: Dict, symbole_test_data: Dict
):
    """Test la création d'un symbole"""
    response = client.post(
        "/api/symboles/", headers=auth_headers, json=symbole_test_data
    )
...
def test_update_symbole(
    ...
    update_data = {"nom": "Symbole Modifié", "description": "Description modifiée"}

    response = client.put(
        f"/api/symboles/{symbole.id}", headers=auth_headers, json=update_data
    )
    assert response.status_code == 200
...
def test_delete_symbole(
    ...
    deleted_symbole = (
        db_session.query(SymboleTag).filter(SymboleTag.id == symbole.id).first()
    )
