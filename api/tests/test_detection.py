...
class TestDetection:
    def test_detect_image_success(self, client, mocker):
        ...
        mocker.patch(
            'api.routes.detection.predict_symbol',
            return_value=("test_symbol", 0.8)
        )
        
        # Mock du seuil de similarité
        mocker.patch(
            'api.routes.detection.templates.similarity_threshold',
            0.65
        )
    ...
    @pytest.mark.parametrize("extension", ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff'])
    def test_valid_extensions(self, client, mocker, extension):
        ...
        response = client.post(
            "/api/detect",
            files={"file": (f"test{extension}", img_bytes, "image/png")}
        )
