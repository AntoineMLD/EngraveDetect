...
class TestGetSerie:
    def test_get_serie_success(self, client, db_session, mock_serie):
        # Arrange
        db_session.query.return_value.filter.return_value.first.return_value = mock_serie
...
class TestUpdateSerie:
    def test_update_serie_success(self, client, db_session, mock_serie, auth_headers):
        ...
        # Act
        response = client.put(f"/api/series/{mock_serie.id}", json=update_data, headers=auth_headers)
