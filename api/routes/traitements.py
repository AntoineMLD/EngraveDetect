...
def get_traitements(db: Session = Depends(get_db)):
    try:
        ...
        return [TraitementResponse(id=t.id, nom=t.nom, type=t.type) for t in traitements]
...
def get_traitement(traitement_id: int, db: Session = Depends(get_db)):
    try:
        ...
        return TraitementResponse(id=traitement.id, nom=traitement.nom, type=traitement.type)
    except SQLAlchemyError as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.post("/traitements", response_model=TraitementResponse, status_code=status.HTTP_201_CREATED)
async def create_traitement(
    ...
    try:
        ...
        return TraitementResponse(id=db_traitement.id, nom=db_traitement.nom, type=db_traitement.type)
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
