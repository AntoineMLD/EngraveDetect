...
class ScrapersPipeline:
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            spider.logger.info(f"État processed_urls avant ajout: {self.processed_urls}")
            self.processed_urls.add(source_url)
            spider.logger.info(f"État processed_urls après ajout: {self.processed_urls}")
            ...
            if image_url:
                ...
                # Si c'est une string et que c'est une URL
                    ...
                    if retry_count >= max_retries:
                        ...
                        spider.logger.error(f"Échec téléchargement image après {max_retries} tentatives")
                else:
                    item["image_engraving"] = None
            else:
                item["image_engraving"] = None
            ...
            spider.logger.info(f"Item ajouté pour {source_url}. Total items: {len(self.items_by_url[source_url])}")
    ...
    def close_spider(self, spider):
        ...
        spider.logger.info(f"Contenu items_by_url: {[f'{url}: {len(items)}' for url, items in self.items_by_url.items()]}")
        ...
        for url in spider.start_urls:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    retry_count += 1
                    spider.logger.error(f"Erreur traitement fichier pour {url} (tentative {retry_count}): {e}")
                    time.sleep(5)

            if retry_count >= max_retries:
                spider.logger.error(f"Échec définitif pour {url} après {max_retries} tentatives")
