...
class GlassSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        supplier_name = response.xpath('/html/body/div[2]/div/div[3]/div[2]/h2/text()').get()
        ...
        for line in lines:
            ...
            nasal_engraving = line.xpath('.//div[contains(@class, "td")][2]//p[@class="gravure_txt"]/b/text()').get()
            nasal_engraving = nasal_engraving or line.xpath('.//div[contains(@class, "td")][2]/img[contains(@src, "nasal")]/@src').get()
            if not nasal_engraving:  
                continue
            item['nasal_engraving'] = nasal_engraving

            # Extraction de l'indice et du matériau
            glass_index = line.css('div.row.tr:not(.group) div.td.col.s1.m1 p::text').get()
