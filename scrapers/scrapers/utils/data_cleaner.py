...
class NettoyeurDonneesOptiques:
    ...
    def nettoyer_materiau(self, materiau: str) -> str:
        ...
        try:
            ...
            return self.MAPPING_MATERIAUX.get(materiau_clean, self.VALEURS_DEFAUT['materiau'])
    ...
    def analyser_nom_verre(self, nom_verre: str) -> Dict[str, Any]:
        ...
        if pd.isna(nom_verre):
            return {k: v for k, v in self.VALEURS_DEFAUT.items() if k in [
                'nom_du_verre', 'gamme', 'serie', 'variante', 'hauteur_min', 
                'hauteur_max', 'traitement_protection', 'traitement_photochromique'
            ]}
        ...
        try:
            ...
            traitement_protection = 'OUI' if 'Eye Protect System' or 'Blue Natural' or 'UVBlue' in nom_verre else 'NON'
            traitement_photochromique = 'OUI' if 'Transitions' or 'Trans®' in nom_verre else 'NON'
            ...
            variante = '|'.join(variantes) if variantes else self.VALEURS_DEFAUT['variante']
    ...
    def nettoyer_indice(self, indice_val: Any) -> list:
        ...
        if not isinstance(indice_val, (str, bytes)):
            self.logger.warning(f"Type d'indice non géré : {type(indice_val)}, valeur: {indice_val}")
            return [self.VALEURS_DEFAUT['indice']]
            ...
            for part in parts:
                ...
                if part:
                    try:
                        ...
                        self.logger.warning(f"Valeur d'indice invalide : {part} dans la chaîne originale : {indice_val}")
                        return [self.VALEURS_DEFAUT['indice']]
            if not indices:
                return [self.VALEURS_DEFAUT['indice']]
            return indices
        except Exception as e:
            self.logger.warning(f"Erreur inattendue lors du nettoyage de l'indice : {e}")
            return [self.VALEURS_DEFAUT['indice']]
    ...
    def nettoyer_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df_propre['traitement_protection'] = noms_traites.apply(lambda x: x['traitement_protection'])
            df_propre['traitement_photochromique'] = noms_traites.apply(lambda x: x['traitement_photochromique'])
            ...
            df_propre['fournisseur'] = df['glass_supplier_name'].fillna(self.VALEURS_DEFAUT['fournisseur'])
            df_propre['url_gravure'] = df['nasal_engraving'].apply(self.extraire_url_image)
            df_propre['url_gravure'] = df_propre['url_gravure'].apply(self.nettoyer_gravure_text)
            df_propre['url_source'] = df['source_url'].fillna(self.VALEURS_DEFAUT['url_source'])

            # Validation des données
            df_propre.loc[~df_propre['indice'].between(1.4, 1.9), 'indice'] = self.VALEURS_DEFAUT['indice']
    ...
    def nettoyer_fichier(self, chemin_entree: Path, dossier_sortie: Path):
        ...
        try:
            ...
            self.logger.error(f"Erreur lors du traitement de {chemin_entree.name}: {str(e)}")
            raise
...
def main():
    ...
    fichiers_entree = [x for x in staging_folder.iterdir() if x.is_file() and x.suffix == '.csv']
